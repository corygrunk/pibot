...
var client = function () {
  ...
  ws.on('message', function incoming(message) {
    ...
    if (message == '!' && timeNow >= 7 && timeNow <= 22) { // Quiet between 10PM and 7AM
      console.log('Pushover: A new message has arrived; you should perform a sync.');
      notify(function (data) {
        console.log(data);
        if (data && data.length > 0 && data[0].app === 'IFTTT') {
          data[0].app = 'If This Then That';
        }
        if (data && data.length > 0 && data[0].app === 'NzbDrone') {
          data[0].app = 'Plex';
          var tvPattern = /.+?(?= - )/;
          var showName = data[0].message.match(tvPattern);
          var tvMsg = 'I have downloaded a new episode of ' + showName + '.';
          data[0].message = tvMsg;
        }
        if (data && data.length > 0 && data[0].app === 'CouchPotato') {
          data[0].app = 'Plex';
          var cpPattern = /(.*)(?=[$\(])/;
          var cpMovieName = data[0].message.match(cpPattern);
          var cpMsg = 'I have ' + cpMovieName + '.';
          data[0].message = cpMsg;
        }
        leds.blink(0,0,1);
        tts.say('Excuse me. You have a new notification. ' + data[0].message);
